...
def analyze_results(results_path: str, activations_path: str, metric: str = "l2"):
    ...
    1. Per-token alignment curves for each instruction (showing instruction vs response alignment)
    2. Aggregate alignment vs N plot showing how alignment changes with more demonstrations
    3. Heatmap of alignment over tokens vs N
    """
    ...
    # ---- Plot 1: Per-token alignment curves for each instruction ----
    fig1, axes1 = plt.subplots(2, min(5, len(results)), figsize=(20, 8))
    if len(results) == 1:
        axes1 = np.array([[axes1[0]], [axes1[1]]])
    
    for idx, result in enumerate(results[:5]):  # Plot first 5 instructions
        if "error" in result:
            continue
        
        ax_top = axes1[0, idx] if len(results) > 1 else axes1[0, 0]
        ax_bot = axes1[1, idx] if len(results) > 1 else axes1[1, 0]
        
        instruction_len = result.get("instruction_seq_len", 0)
        
        # No prefix alignment
        if "alignment_per_token_no_prefix" in result:
            alignment = result["alignment_per_token_no_prefix"]
            token_positions = np.arange(len(alignment))
            ax_top.plot(token_positions, alignment, label="N=0", alpha=0.7, linewidth=2)
            ax_top.axvline(x=instruction_len, color='red', linestyle='--', alpha=0.5, label='Response start')
        ...
        if "manyshot_results" in result:
            ...
            for i, (n_demos, manyshot_result) in enumerate(sorted(result["manyshot_results"].items(), key=lambda x: int(x[0]))):
                if "alignment_per_token" in manyshot_result:
                    alignment = manyshot_result["alignment_per_token"]
                    token_positions = np.arange(len(alignment))
                    ax_bot.plot(token_positions, alignment, label=f"N={n_demos}", 
                               color=colors[i], alpha=0.7, linewidth=1.5)
            
            ax_bot.axvline(x=instruction_len, color='red', linestyle='--', alpha=0.5)
        
        ax_top.set_title(f"Instruction {idx+1}")
        ax_top.set_ylabel(alignment_label)
        ax_top.legend(loc='lower right', fontsize=8)
        ax_top.grid(True, alpha=0.3)
        
        ax_bot.set_xlabel("Token Position")
        ax_bot.set_ylabel(alignment_label)
        ax_bot.legend(loc='lower right', fontsize=8)
        ax_bot.grid(True, alpha=0.3)
    
    fig1.suptitle("Per-Token Alignment: Steered vs Natural Activations", fontsize=14)
    fig1.tight_layout()
    
    plot1_path = os.path.join(output_dir, f"per_token_alignment_{metric}.png")
    fig1.savefig(plot1_path, dpi=150, bbox_inches='tight')
    print(f"Per-token plot saved to: {plot1_path}")
    
    # ---- Plot 2: Aggregate alignment vs N ----
    ...
    for result in results:
        ...
        if "alignment_instruction_mean_no_prefix" in result:
            ...
            instr_alignment_by_n[0].append(result["alignment_instruction_mean_no_prefix"])
            if result.get("alignment_response_mean_no_prefix") is not None:
                resp_alignment_by_n[0].append(result["alignment_response_mean_no_prefix"])
            if "alignment_per_token_no_prefix" in result:
                overall_alignment_by_n[0].append(np.mean(result["alignment_per_token_no_prefix"]))
    ...
    instr_means = [np.mean(instr_alignment_by_n[n]) for n in n_values]
    instr_stds = [np.std(instr_alignment_by_n[n]) for n in n_values]
    resp_means = [np.mean(resp_alignment_by_n[n]) if resp_alignment_by_n[n] else 0 for n in n_values]
    resp_stds = [np.std(resp_alignment_by_n[n]) if resp_alignment_by_n[n] else 0 for n in n_values]
    ...
    ax2.errorbar(n_values, instr_means, yerr=instr_stds, 
                marker='o', capsize=5, capthick=2, linewidth=2, markersize=8,
                label='Instruction portion')
    ax2.errorbar(n_values, resp_means, yerr=resp_stds, 
                marker='s', capsize=5, capthick=2, linewidth=2, markersize=8,
                label='Response portion')
    ax2.errorbar(n_values, overall_means, yerr=overall_stds, 
                marker='^', capsize=5, capthick=2, linewidth=2, markersize=8,
                label='Overall')
    
    ax2.set_xlabel("Number of ICL Demonstrations (N)", fontsize=12)
    ax2.set_ylabel(f"Mean {alignment_label}", fontsize=12)
    ax2.set_title("Activation Alignment vs. Number of Many-Shot Demonstrations", fontsize=14)
    ax2.set_xscale('symlog', linthresh=1)
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    
    plot2_path = os.path.join(output_dir, f"alignment_vs_n_demos_{metric}.png")
    fig2.savefig(plot2_path, dpi=150, bbox_inches='tight')
    print(f"Alignment vs N plot saved to: {plot2_path}")
    
    # ---- Plot 3: Heatmap of alignment over tokens vs N ----
    # Average per-token alignment across all instructions for each N
    max_tokens = 0
    for result in results:
        if "alignment_per_token_no_prefix" in result:
    ...
    if max_tokens > 0:
        n_list = sorted([n for n in overall_alignment_by_n.keys()])
        alignment_matrix = np.full((len(n_list), max_tokens), np.nan)
        ...
        for row_idx, n in enumerate(n_list):
            ...
            for result in results:
                ...
                if n == 0:
                    if "alignment_per_token_no_prefix" in result:
                        align = result["alignment_per_token_no_prefix"]
        ...
        if metric == "cosine":
            im = ax3.imshow(alignment_matrix, aspect='auto', cmap='RdYlGn', vmin=-1, vmax=1)
        else:
            im = ax3.imshow(alignment_matrix, aspect='auto', cmap='viridis_r', vmin=None, vmax=None)
        ...
        ax3.set_title("Activation Alignment Heatmap: Token Position vs N", fontsize=14)
        plt.colorbar(im, ax=ax3, label=alignment_label)
        
        plot3_path = os.path.join(output_dir, f"alignment_heatmap_{metric}.png")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default='Qwen/Qwen2.5-0.5B-Instruct', help="Model to use")
    ...
    parser.add_argument("--analyze", type=str, default="/src/new_cont/llms/steercheck/invertsteer/outputs/Qwen2.5-0.5B-Instruct/manyshot/manyshot_results_refusal_actadd_coeff_-1.0.json",
